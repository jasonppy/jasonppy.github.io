...
Combine step 2 and 3, we see that fitted Q-iteration is actually looking for $$\phi$$ that minimizes
...
Last thing to point out before we leave this section is that the Q-iteration algorithm is off-policy, the policy is evolving all the time when we  do $$\max_{a}Q_{\phi}(s,a)$$ and the fitting of Q-function requires only valid transition tuples $$(s,a,s')$$, which does not need to be generated by the newest policy.
